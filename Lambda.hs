...
tiExpr g (If s t e) = do (a, s1) <- tiExpr g s
                         ...
                         (b, s2) <- tiExpr (apply u1 g) t
                         (c, s3) <- tiExpr (apply s2 g) e 
                         let u2 = unify (apply s3 b) (c) 
                         return (apply u2 b, s3 @@ s2 @@ s1 @@ u1 @@ u2) 
...
ex7 = Lam "x" (Lam "y" (If (App (App (Var "==") (Var "x")) (Var "y")) (Var "True") (Var "False")))
